# .cwd <- getwd()
# setwd(here::here()) # need install in conda
# source("renv/activate.R") # need install conda 
# options(renv.config.pak.enabled = FALSE) # Do not use pak!
library(reticulate)
...
convert.h5mu <- function(mdata) {
    builtins <- reticulate::import_builtins()
    mod <- list()
    for (key in builtins$list(mdata$mod$keys())) {
        mod[[key]] <- convert.h5ad(mdata$mod[[key]])
    }
    obs <- .convert(mdata$obs)
    var <- .convert(mdata$var)
    obsm <- .convert(mdata$obsm)
    varm <- .convert(mdata$varm)
    obsp <- .convert(mdata$obsp)
    varp <- .convert(mdata$varp)
    uns <- .convert(mdata$uns)
    return(list(
        mod = mod,
        obs = obs, var = var,
        obsm = obsm, varm = varm,
...
read.h5mu <- function(filename) {
    md <- reticulate::import("mudata", convert = FALSE)
    mdata <- md$read_h5mu(filename)
    return(convert.h5mu(mdata))
}
